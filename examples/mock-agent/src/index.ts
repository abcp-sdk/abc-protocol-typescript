...
export async function runMockAgent(agent: Agent): Promise<void> {
  ...
  const echo = await agent.callTool('demo', 'sess-1', 'mock-ext', 'echo', 'c1', { msg: 'hello' })
  console.log('[mock-agent] echo:', echo.content)

  const add = await agent.callTool('demo', 'sess-1', 'mock-ext', 'add', 'c2', { a: 2, b: 3 })
  console.log('[mock-agent] add:', JSON.stringify(add.data))

  const url = await agent.resolveVariable('demo', 'mock-ext', 'base-url')
  console.log('[mock-agent] base-url:', url)

  const hook = await agent.callHook('demo', 'sess-1', 'mock-ext', 'session.before_create', { org: 'x' })
